#!/bin/bash
# VibesDeGoGo! hook 欠陥修正 検証ハーネス
#
# 設計: 「修正後のあるべき挙動」をアサートする。
#   - 未修正スクリプト → 回帰(R*)=PASS / 欠陥(A/B/C/D/E/INIT)=FAIL（バグ存在の証明＝baseline）
#   - 修正完了スクリプト → 全 PASS
#
# 被験体: skills/vibesdegogo/scripts/ の各 hook / vdgg-state.sh（repo 内、guard2 リテラルは[VibesDeGoGo! Step N Start]）
# 実機・ビルド不要。stdin JSON で hook を駆動し exit code を観測する。

set -u

REPO="$(cd "$(dirname "$0")/../../../.." && pwd)"
SCRIPTS="$REPO/skills/vibesdegogo/scripts"
HOOK_PRE="$SCRIPTS/vdgg-hook-pretool.sh"
HOOK_POST="$SCRIPTS/vdgg-hook-posttool.sh"
STATE_SH="$SCRIPTS/vdgg-state.sh"
...
# 擬似 cwd を作り .claude/.vdgg-active + state file を配置。echo で cwd を返す。
mk_cwd() { # step phase loop current_task
  local d; d=$(mktemp -d)
  mkdir -p "$d/.claude" "$d/tasks/vdgg/$ID"
  echo "$ID" > "$d/.claude/.vdgg-active"
  cat > "$d/.claude/.vdgg-state-$ID" <<EOF
step=$1
phase=$2
loop_count=$3
current_task=${4:-}
...
: > "$CWD/tasks/vdgg/$ID/requirements.md"
JSON=$(j --arg cwd "$CWD" --arg cmd '# [VibesDeGoGo! Step 3 Start] step=3
vdgg_state_advance 3 investigating' '{tool_name:"Bash",cwd:$cwd,tool_input:{command:$cmd}}')
...
sleep 1; : > "$CWD/tasks/vdgg/$ID/progress.md"   # state より新しい mtime
: > "$CWD/tasks/vdgg/$ID/investigation-r1.md"
JSON=$(j --arg cwd "$CWD" --arg cmd '# [VibesDeGoGo! Step 6 Start] step=6
vdgg_state_loop 6 implementing' '{tool_name:"Bash",cwd:$cwd,tool_input:{command:$cmd}}')
...
cat > "$CWD/.claude/.vdgg-simplify-sentinel-$ID-0" <<EOF
started=1
modified=0
modified_files=
EOF
JSON=$(j --arg cwd "$CWD" --arg cmd '# [VibesDeGoGo! Step 7 Start] step=7
vdgg_state_advance 7 verified' '{tool_name:"Bash",cwd:$cwd,tool_input:{command:$cmd}}')
...
: > "$CWD/tasks/vdgg/$ID/progress.md"; sleep 1
: > "$CWD/tasks/vdgg/$ID/investigation-r1.md"
touch "$CWD/.claude/.vdgg-state-$ID"   # state を後で touch → progress が古い
JSON=$(j --arg cwd "$CWD" --arg cmd '# [VibesDeGoGo! Step 6 Start] step=6
vdgg_state_loop 6 implementing' '{tool_name:"Bash",cwd:$cwd,tool_input:{command:$cmd}}')
...
CWD=$(mk_cwd 2 requirements 0 "")
JSON=$(j --arg cwd "$CWD" --arg fp "$CWD/tasks/vdgg/$ID/requirements.md" '{tool_name:"Write",cwd:$cwd,tool_input:{file_path:$fp}}')
chk "D requirements で TASKS_DIR 内 Write は通る(回帰)" "$(run_hook "$HOOK_PRE" "$JSON")" 0

CWD=$(mk_cwd 1 declare 0 "")
JSON=$(j --arg cwd "$CWD" --arg cmd '# [VibesDeGoGo! Step 2 Start] step=2
vdgg_state_advance 2 requirements' '{tool_name:"Bash",cwd:$cwd,tool_input:{command:$cmd}}')
chk "D declare で Bash(vdgg_state_*) は通る(回帰)" "$(run_hook "$HOOK_PRE" "$JSON")" 0
...
echo "=== E: 8→5 で loop_count=0 リセット（vdgg-state.sh）==="
D=$(mktemp -d)
(
  export VDGG_CWD="$D"
  source "$STATE_SH"
...
EOF
  ...
  grep '^loop_count=' "$D/.claude/.vdgg-state-$(cat "$D/.claude/.vdgg-active")" | cut -d= -f2
...
echo "=== INIT: 旧残骸掃除（vdgg_state_init）==="
D=$(mktemp -d)
(
  export VDGG_CWD="$D"
  source "$STATE_SH"
  mkdir -p "$D/.claude"
  echo "old" > "$D/.claude/.vdgg-active"
  : > "$D/.claude/.vdgg-error-pending"
  : > "$D/.claude/.vdgg-simplify-sentinel-old-0"
  : > "$D/.claude/.vdgg-step-block-old"
  vdgg_state_init >/dev/null 2>&1
  remain=0
  [ -f "$D/.claude/.vdgg-error-pending" ] && remain=$((remain+1))
  ls "$D/.claude/".vdgg-simplify-sentinel-* >/dev/null 2>&1 && remain=$((remain+1))
  ls "$D/.claude/".vdgg-step-block-* >/dev/null 2>&1 && remain=$((remain+1))
  echo "$remain"
